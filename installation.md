...
](http://www.telerik.com/blogs/optimizing-use-of-the-telerik-control-panel
)−To download and install the Control Panel, log into your
            
[Telerik account
](http://www.telerik.com/account/default.aspx
). Telerik Control Panel uses the product MSI file.
...
)−The Telerik Reporting NuGet packages contain the libraries for .NET Core projects, the Reporting Engine, and the implementation of the
            
[Reporting REST WebAPI-based service]({%slug telerikreporting/using-reports-in-applications/host-the-report-engine-remotely/telerik-reporting-rest-services/asp.net-web-api-implementation/overview%})
 where you can use the packages
...
__ > 
__Progress
__ folder. They also create the necessary
...
1. Click the 
__Downloads
__ tab to view the available trial and commercial products.
...
1. For the latest internal builds, navigate to 
...
__Examples
__ dialog option will be installed but the configuration files for the application will not have valid connection strings.
...
* To set up the examples and the used connection string, select 
__Examples Setup
__. All local SQL Server 2005+ instances are supported.
                


* The examples use the MS SQL Server 
...
[download AdventureWorks from codeplex
...
__[InstallDir]/Examples/Data/AdventureWorks OLTP
__.
                  For the script to run correctly, uncomment the 
...
__Examples Setup
__ was selected.) In the 
__Setup Wizard
__ dialog, select your
...
`                C:\Program Files\Progress\Telerik Reporting 
{{site.suiteversion
}}`.
