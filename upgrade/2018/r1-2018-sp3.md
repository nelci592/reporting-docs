...
In all the REST Report Service implementations (WebAPI, Service Stack, ASP.NET Core), the HttpContext.Current instance is not accessible in the rendering thread.
              Instead the rendering thread makes use of the new property [Telerik.Reporting.Processing.UserIdentity.Current](/reporting/api/Telerik.Reporting.Processing.UserIdentity#collapsible-Telerik_Reporting_Processing_UserIdentity_Current), which is also available in the 
              report expression context as a global object: `=UserIdentity`. All the reports that rely on __HttpContext.Current__, 
              must replace its usage with the new dedicated property.
